 ...
 * against the configured backend (default: SQLite) using a temporary database.
 */
...
// Point STORE_DIR / SEEKDB_PATH to a temp directory so we don't touch the real DB
const tempDir = mkdtempSync(path.join(tmpdir(), 'nanoclaw-verify-'));
const backend = process.env.DB_TYPE || 'sqlite';
if (backend === 'seekdb') {
  if (!process.env.SEEKDB_PATH && !process.env.SEEKDB_HOST) {
    process.env.SEEKDB_PATH = tempDir;
  }
  if (!process.env.SEEKDB_DATABASE) {
    process.env.SEEKDB_DATABASE = 'nanoclaw_verify';
  }
} else {
...
async function run() {
  console.log(`\n=== Adapter integration verification (backend: ${backend}) ===\n`);
  ...
  // 8. Upsert behavior (SQLite vs seekdb dialect difference)
  console.log('\n[8] Upsert behavior');
  await db.setRegisteredGroup('verify@g.us', {
    name: 'Renamed Group',
    folder: 'verify',
