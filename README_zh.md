...
单一 Node.js 进程。智能体在具有挂载目录的隔离 Linux 容器中执行。每个群组的消息队列都带有全局并发控制。通过文件系统进行进程间通信（IPC）。数据库后端可插拔——SQLite（默认）或 seekdb——通过 `DB_TYPE` 环境变量选择。
...
- `src/db/` - 数据库适配层（SQLite 和 seekdb 后端）
